...
Na pocz¹tku rozdzia³u przedstawiono pod³o¿e teoretyczne pracy. Nastêpnie dokonano porównania miêdzy koncepcj¹ pierwotn¹, a finaln¹ architektury systemu. W kolejnych podrozdzia³ach zawarto uzasadnienie decyzji projektowych finalnej struktury systemu oraz wyjaœniono podstawy technologii wybranych do realizacji poszczególnych zadañ. Implementacje opisano w~rozdziale~\ref{chap:koncepcja}.

\section{Pod³o¿e teoretyczne}
W poni¿szej sekcji przedstawiono pod³o¿e teoretyczne trzech zagadnieñ: komputer rowerowy, GPS oraz GPX. Koncepcje te stoj¹ u podstaw ca³ej pracy. Z tego te¿ wzglêdu, zaznajomienie siê z poni¿szymi informacjami jest wskazane przed przyst¹pieniem do lektury dalszych rozdzia³ów.
...
\begin{enumerate}
  \item Wspó³czeœnie znakomita wiêkszoœæ osób jest posiadaczem smartfona~\cite{smartphones}.
  \item Nie wymaga inwestycji finansowej (chocia¿ aplikacje zazwyczaj oferuj¹ dodatkowe, rozbudowane, p³atne funkcjonalnoœci).
\end{enumerate}
...
Kluczowe jest rozró¿nienie miêdzy \emph{route type} i \emph{track type}, pierwszy z nich wytycza jedynie punkty poœrednie prowadz¹ce do celu, drugi z nich to zapis rzeczywistych danych zebranych przez fizyczne urz¹dzenie. Poniewa¿ polskie t³umaczenia tych angielskich terminów mog¹ nie byæ w pe³ni jednoznaczne, bezpieczniej jest pos³ugiwaæ siê angielskimi formami. Aby unikn¹æ nieporozumieñ, w dalszej treœci pracy przyjêto nastêpuj¹c¹ konwencjê nazewnicz¹: \emph{route} --- trasa, \emph{track} --- œlad.

Szablonow¹ strukturê standardu GPX~\cite{gpxschema} przedstawiono na listingu~\ref{lst:gpxschema} natomiast na listingu~\ref{lst:gpxexample} pokazano zawartoœæ przyk³adowego pliku (fragment rzeczywistych danych zebranych przez autora pracy, wyeksportowanych z aplikacji mobilnej \texttt{Komoot}). Z listingu mo¿na odczytaæ, ¿e plik zawiera jeden \emph{track}, która sk³ada siê z jednego segmentu (\texttt{trkseg}) z list¹ punktów (\texttt{trkpt}). Lista ta na potrzeby demonstracji zosta³a skrócona do trzech punktów, jednak w rzeczywistoœci, w zale¿noœci od d³ugoœci \emph{tracka}, mo¿e ona liczyæ setki, a nawet tysi¹ce punktów.

{\belowcaptionskip=-9pt
\begin{lstlisting}[language=XML, label=lst:gpxschema, caption=Struktura pliku GPX, basicstyle=\footnotesize\ttfamily]
<gpx
 version="1.1 [1] ?"
...
{\belowcaptionskip=-9pt
\begin{lstlisting}[language=XML, label=lst:gpxexample, caption=Przyk³adowy plik GPX, basicstyle=\footnotesize\ttfamily]
...
Dziêki otwartoœci standardu mo¿liwe jest jego rozszerzanie. Jednym z popularnych rozszerzeñ jest \emph{Garmin GPX Schema Extension}. Dodaje ono nowe typy danych, które pozwalaj¹ zapisaæ w pliku dodatkowe informacje. Przyk³adowo, w omówionym wczeœniej \emph{waypoint type} mo¿na utrwaliæ zarejestrowan¹ w danym punkcie temperaturê lub kategoriê do której dany punkt nale¿y. Z kolei \emph{track type} zyskuje mo¿liwoœæ przechowywania koloru, w jakim œlad powinien zostaæ wyœwietlony na mapie. Wszystkie funkcjonalnoœci rozszerzenia opisano w oficjalnym~schemacie~\cite{gpxgarmin}.

\section{Zarys wymagañ}
W wyniku zebrania wymagañ projektowych powsta³a lista ogólna lista wymagañ u¿ytkowych, któr¹ podzielono na dwie kategorie: podstawowe oraz dodatkowe. Spe³nienie wymagañ z~kategorii podstawowych uznano za równoznaczne z osi¹gniêciem wyznaczonego celu pracy. Kategorii wymagañ dodatkowych nie przewidziano do realizacji w ramach pracy. Przedstawia ona jedynie pomys³y i kierunek dalszego rozwoju. Kompletna lista jest nastêpuj¹ca:
\begin{enumerate}
  \item Wymagania podstawowe:
        \begin{itemize}
        ...
        \end{itemize}
  \item Wymagania dodatkowe:
        \begin{itemize}
          \item Edycja profilu (np.:~zmiana danych personalnych).
...
Na poni¿szej liœcie zawarto szczegó³owe wymagania funkcjonalne aplikacji:
\begin{enumerate}
  \item Uwierzytelnianie:
        \begin{enumerate}
          \item Rejestracja z u¿yciem adresu email i has³a.
          \item Logowanie z u¿yciem adresu email i has³a.
          \item Autoryzacja z u¿yciem konta \texttt{Google}.
          \item Wylogowanie z aplikacji.
        \end{enumerate}
  \item Nagrywanie œladów GPS:
        \begin{enumerate}
          \item Podgl¹d mapy.
          \item W³¹czenie rejestrowania œladu GPS.
          \item Wy³¹czenie rejestrowania œladów GPS.
          \item Zapisanie zarejestrowanego œladu GPS jako nowa aktywnoœæ.
        \end{enumerate}
  \item Transfer danych:
        \begin{enumerate}
          \item Import danych w standardzie GPX.
          \item Eksport danych w standardzie GPX.
        \end{enumerate}
  \item Aktywnoœci:
        \begin{enumerate}
          \item Dodawanie nowych aktywnoœci: import GPX lub rejestrator GPS.
          \item Usuwanie aktywnoœci.
          \item Edycja aktywnoœci:
                \begin{itemize}
                  \item nazwa;
                  \item rodzaj sportu;
                  \item rodzaj kategorii;
                  \item kszta³t œladu aktywnoœci --- pêtla lub od punktu do punktu, punkt pocz¹tkowy, punkt koñcowy;
                  \item ocena;
                  \item tagi.
                \end{itemize}
          \item Wyœwietlanie danych aktywnoœci:
                \begin{itemize}
                  \item nazwa;
                  \item rodzaj sportu;
                  \item rodzaj kategorii;
                  ...
                  \item tagi.
                \end{itemize}
          \item Wyœwietlanie statystyk aktywnoœci:
                \begin{itemize}
                  \item data utworzenia;
                  \item data ostatnie modyfikacji;
                  \item data pocz¹tku aktywnoœci;
                  \item data koñca aktywnoœci;
                  \item czas spêdzony w ruchu;
                  \item ca³kowity czas trwania;
                  \item dystans ca³kowity;
                  \item prêdkoœæ maksymalna;
                  \item prêdkoœæ œrednia w ruchu;
                  \item prêdkoœæ œrednia ca³kowita;
                  \item przewy¿szenia pokonane w górê;
                  \item przewy¿szenia pokonane w dó³;
                  \item najni¿sze zarejestrowane po³o¿enie nad poziomem morza;
                  \item najwy¿sze zarejestrowane po³o¿enie nad poziomem morza.
                \end{itemize}
          \item Wyœwietlanie wykresów aktywnoœci:
                \begin{itemize}
                  \item zale¿noœæ prêdkoœci od czasu;
                  \item zale¿noœæ pokonanego dystansu od czasu;
                  \item zale¿noœæ bezwzglêdnej wysokoœci nad poziomem morza od czasu.
                \end{itemize}
        \end{enumerate}
  \item Profil u¿ytkownika:
        \begin{enumerate}
          \item Wyœwietlanie danych profilu:
                \begin{itemize}
          ...
          \item Edycja danych profilu:
                \begin{itemize}
                  ...
                  \item miasto zamieszkania.
                \end{itemize}
          \item Wyœwietlanie statystyk profilu:
                \begin{itemize}
                  \item ca³kowita liczba aktywnoœci;
                  \item maksymalna zarejestrowana prêdkoœæ;
                  \item data pierwszej aktywnoœci;
                  \item data ostatniej aktywnoœci;
                  \item ca³kowity czas aktywnoœci (czas ca³kowity);
                  \item œredni czas trwania aktywnoœci (czas ca³kowity);
                  \item najkrótsza aktywnoœæ (czas ca³kowity);
                  \item najd³u¿sza aktywnoœæ (czas ca³kowity);
                  \item œrednia prêdkoœæ (czas ca³kowity);
                  \item ca³kowity czas aktywnoœci (czas w ruchu);
                  \item œredni czas trwania aktywnoœci (czas w ruchu);
                  \item najkrótsza aktywnoœæ (czas w ruchu);
                  \item najd³u¿sza aktywnoœæ (czas w ruchu);
                  \item œrednia prêdkoœæ (czas w ruchu);
                  \item ca³kowity dystans;
                  \item œredni dystans;
                  \item najkrótszy dystans;
                  \item najd³u¿szy dystans;
                  \item suma przewy¿szeñ w górê;
                  \item suma przewy¿szeñ w dó³;
                  \item najni¿sze zarejestrowane po³o¿enie nad poziomem morza;
                  \item najwy¿sze zarejestrowane po³o¿enie nad poziomem morza.
                \end{itemize}
          \item Wyœwietlanie wykresów profilu:
                \begin{itemize}
...
W poni¿szej liœcie zawarto wymagania niefunkcjonalne aplikacji:
\begin{enumerate}
  \item Dostêp do aplikacji z przegl¹darki komputera osobistego.
  \item Dostêp do aplikacji z przegl¹darki urz¹dzenia mobilnego.
  \item Synchronizacja danych niezale¿na od urz¹dzenia.
  \item Mo¿liwoœæ instalacji aplikacji na urz¹dzeniu mobilnym.
  \item Responsywnoœæ aplikacji.
  \item Przejrzystoœæ dzia³ania aplikacji.
  \item Przyjaznoœæ interfejsu u¿ytkownika.
  \item Interfejs u¿ytkownika w jêzyku angielskim.
  \item Szybkoœæ dzia³ania.
  \item Wydajnoœæ dzia³ania.
  \item Bezpieczeñstwo danych u¿ytkownika.
\end{enumerate}


\section{Diagram przypadków u¿ycia}
Diagram przypadków u¿ycia pokazano na rysunku~\ref{fig:uzycia}

\begin{figure}[hp]
  \centering
  \includegraphics[width=0.9\linewidth]{rys02/uzycia}
  \caption{Diagram przypadków u¿ycia}
  \label{fig:uzycia}
\end{figure}

\section{Architektura systemu}
Pierwotna koncepcja zak³ada³a zaprojektowanie i implementacjê dwóch osobnych aplikacji pod wspóln¹ nazw¹. Pierwsz¹ z nich mia³a byæ aplikacja internetowa (dostêp poprzez przegl¹darkê). W niej zawarte mia³y byæ wszystkie planowane funkcjonalnoœci za wyj¹tkiem jedynie nagrywania œladów z wykorzystaniem modu³u GPS. Za to zadanie mia³a byæ odpowiedzialna dedykowana aplikacja mobilna dla systemu \texttt{Android}. Ta wizja jednak uleg³a zmianie --- finalna wersja zak³ada wykonanie aplikacji internetowej w technice PWA. Pozwala ona na instalacjê aplikacji internetowej na urz¹dzeniu mobilnym i sprawienie wra¿enia aplikacji natywnej. Takie rozwi¹zanie posiada zarówno swoje zalety, jak i wady.

\clearpage
Do korzyœci na pewno nale¿¹:
\begin{enumerate}
...
Porównanie koncepcji oraz dobór technologii realizacji poszczególnych modu³ów systemu zestawiono w tabeli~\ref{tab:koncepcje}. Na rysunku~\ref{fig:architektura} przedstawiono schemat ostatecznej wersji architektury systemu. Szczegó³y opisano w kolejnych sekcjach.
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.9\linewidth]{rys02/architektura}
...
Ca³a warstwa backend realizowanego projektu oparta jest na platformie \texttt{Google Firebase}. W jej zakres wchodz¹ modu³y, które przedstawiono w tabeli~\ref{tab:firebase}. Szczegó³y mo¿na znaleŸæ w~dokumentacji technicznej \cite{firebase}. G³ówne powody wyboru akurat tej platformy s¹ nastêpuj¹ce:
\begin{itemize}
...
\begin{table}[htb] \small
  \centering
  \caption{Modu³y platformy \texttt{Firebase}}
  \label{tab:firebase}
...
W ramach projektu wykorzystane zosta³y modu³y: \texttt{Authentication}, \texttt{Realtime Database} i \texttt{Hosting}. W kolejnych sekcjach opisano  zwi¹zane z nimi za³o¿enia.
...
Schemat struktury obiektu JSON stanowi¹cego bazê danych przedstawia pseudokod zawarty na listingu~\ref{lst:database}. Na schemacie podano typy poszczególnych pól. Znak~\texttt{?} przy nazwie pola sugeruje, ¿e pole to mo¿e byæ w bazie niezainicjalizowane. Wpis \texttt{<unique(userId)>} nale¿y rozumieæ, ¿e w obrêbie obiektu-rodzica mo¿e wyst¹piæ wiele obiektów, których nazwy s¹ unikalne. Typy \texttt{GENDERS}, \texttt{RATINGS}, \texttt{SPORTS}, \texttt{CATEGORIES} pe³ni¹ w schemacie rolê tylko pomocnicz¹, a~zapis \texttt{valueOf(\emph{TYPE})} oznacza, ¿e pole przyjmuje jedn¹ wartoœæ z danego \emph{TYPE}. W przypadku \texttt{activities} i \texttt{tracks} obiekty-dzieci przyjmuj¹ nazwy odpowiadaj¹ce identyfikatorowi aktywnoœci. Wystêpuje tu relacja jeden do jednego --- jeden \emph{track} jest przyporz¹dkowany do jednej \emph{activity}. Zastosowana praktyka jest powszechnie stosowana w~obiektowych bazach danych.

  {\belowcaptionskip=-9pt
    \begin{lstlisting}[language=JavaScript, label=lst:database, caption=Struktura zastosowanej bazy danych w postaci pseudokodu, basicstyle=\footnotesize\ttfamily]
...
{\belowcaptionskip=-9pt
\begin{lstlisting}[language=JavaScript, label=lst:databaserules, caption=Przyk³ad zasady definiuj¹cej dostêp do bazy danych, basicstyle=\footnotesize\ttfamily]
{
  "rules": {
    "users": {
      "$uid": {
...
\begin{table}[htb] \small
  \centering
  \caption{G³ówne biblioteki wykorzystane przy tworzeniu aplikacji klienta}
  ...
  \begin{tabularx}{\linewidth}{|l|X|X|} \hline
    ...
    \texttt{React-Leaflet}                    & Wrapper \texttt{Leafleta}, gotowe komponenty reactowe                                                                        & \url{react-leaflet.js.org}                                             \\      \hline
    ...
    \texttt{useTimeout}, \texttt{useInterval} & \emph{Hooki} reactowe umo¿liwiaj¹ce deklaratywne u¿ycie javscriptowych funkcji \texttt{setTimeout} oraz \texttt{setInterval} & \url{gist.github.com/Danziger/336e75b6675223ad805a88c2} \url{dfdcfd4a} \\      \hline
  \end{tabularx}
\end{table}