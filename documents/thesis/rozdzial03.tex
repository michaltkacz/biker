...
Schemat strony rejestratora GPS przedstawiono na rysunku~\ref{fig:trackerpage}, a widok aplikacji na rysunku~\ref{fig:trackerpageview}. Komponent \texttt{TrackerPage} zawiera trzy komponenty:
\begin{enumerate}
...
Funkcjonalnoœæ obliczania statystyk zaimplementowano w \emph{hooku} \texttt{useActivityStatistics}. \emph{Hook} zwraca obiekt statystyk który zawiera: ostatni¹ (najnowsz¹) oraz maksymaln¹ uzyskan¹ prêdkoœæ; ostatni¹ (najnowsz¹), minimaln¹ i maksymaln¹ wysokoœæ nad poziomem morza, sumê przewy¿szeñ przejechanych pod górê i z góry; ca³kowity przejechany dystans; ca³kowity czas podró¿y oraz czas podró¿y w ruchu.
...
Strona profilu u¿ytkownika sk³ada siê z trzech g³ównych sekcji: sekcja danych u¿ytkownika, sekcja statystyk oraz sekcja wykresów
...
Komponent \texttt{ProfileCharts} odpowiedzialny jest za prezentacjê wykresów profilu u¿ytkownika. Komponent przyjmuje jako w³aœciwoœæ tablicê obiektów \texttt{Activity}. Na pocz¹tku dane aktywnoœci mapowane s¹ na serie danych obs³ugiwane przez wykresy z biblioteki \texttt{React-Vis}. Nastêpnie serie danych przekazywane s¹ do komponentów odpowiednich wykresów, które je formatuj¹ i wyœwietlaj¹. Przyk³adowe wykresy zaprezentowano na rysunku~\ref{fig:profilecharts}.
