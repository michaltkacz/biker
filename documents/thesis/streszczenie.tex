\pdfbookmark[0]{Streszczenie}{streszczenie.1}
%\phantomsection

\begin{abstract}
    Celem pracy jest stworzenie aplikacji internetowej udostêpniaj¹ce fundamentalne funkcje komputera rowerowego. Aplikacja ma byæ przystosowana do dzia³ania zarówno na urz¹dzeniach desktopowych jak i urz¹dzeniach mobilnych. Do podstawowych funkcji aplikacji nale¿¹: uwierzytelnianie u¿ytkownika, nagrywanie œladów przejechanych tras rowerowych z wykorzystaniem modu³u GPS (ang.~\emph{Global Positioning System}), przegl¹danie zapisanych œladów, analiza statystyk zapisanych œladów oraz mo¿liwoœæ importu i eksportu danych z wykorzystaniem standardu GPX (ang.~\emph{GPS Exchange Format}). Wyró¿nikiem tej aplikacji na tle dostêpnych na rynku rozwi¹zañ jest to, ¿e jest to aplikacja internetowa typu PWA (ang.~\emph{Progressive Web App}), co umo¿liwia zainstalowanie jej na urz¹dzeniach mobilnych i sprawienie wra¿enia aplikacji natywnej. Niniejszy dokument zawiera podstawowe wyjaœnienie czym jest komputer rowerowy oraz przedstawia szczegó³y techniczne projektu.
\end{abstract}
\mykeywords{aplikacja internetowa, komputer rowerowy, nawigacja rowerowa, GPS, GPX}\\
% Dobrze by³oby skopiowaæ s³owa kluczowe do metadanych dokumentu pdf (w pliku Dyplom.tex)
% Niestety, zaimplementowane makro nie robi tego z automatu, wiêc pozostaje kopiowanie rêczne.

{
\selectlanguage{english}
\begin{abstract}
    The goal of the work is to create a web application providing the fundamental functions of a cycling computer. The application should be available for both desktop and mobile devices. The basic functions of the application shall be as follows: user authentication, cycling tracks recording using the GPS (\emph{Global Positioning System}), recorded tracks browsing, recorded tracks statistics analysis and the data import and export in the GPX (\emph{GPS Exchange Format}) standard format. The main feature that distinguishes this application from already available products is the use of PWA (\emph{Progressive Web App}), which makes it possible to install web application on mobile devices and simulate the experience of a native mobile application. This document provides a basic explanation of what cycling computer is and provides technical details of the project.
\end{abstract}
\mykeywords{web application, cycling computer, cycling navigation, GPS, GPX}
}
